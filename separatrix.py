...
def separatrix(mu=0.00311):
    """Функция для получения координат точек сепаратрисы при разных предопределенных значениях mu
    в промежутке от 0.00311 до 0.01"""
    ...
    result = main_functions.runge_cutt(grid[mu][0], mu=mu, points_count=2000)
    ...
    result = main_functions.runge_cutt(grid[mu][2], mu=mu, points_count=2000)
    result = list(result)
    result[0].reverse()
    result[1].reverse()
    x_list.extend(result[0])
